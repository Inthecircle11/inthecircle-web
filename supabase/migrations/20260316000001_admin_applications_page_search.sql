-- Add optional search (email, name, username) to admin applications list.
-- When p_search is set, filter applications by matching profile or application fields.
...
COMMENT ON FUNCTION public.admin_get_applications_page(text, text, uuid, int, int, text) IS
  'Admin: paginated applications list with optional search by email, name, username (SECURITY DEFINER).';

-- Count of applications matching the same filters + search (for pagination when searching).
