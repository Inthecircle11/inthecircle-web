-- Add new_users_7d to overview counts so "New (7d)" is server-authoritative and consistent with New (24h).
...
AS $$
  SELECT
    ...
    (SELECT count(*)::bigint FROM auth.users WHERE created_at > now() - interval '7 days'),
...
COMMENT ON FUNCTION public.admin_get_overview_counts() IS 'Admin: dashboard card counts in one query (users, verified, new 24h/7d/30d, threads, messages, apps 7d).';
