...
- **Publish from GitHub (no local `.env`):** Add Actions secrets `WORDPRESS_URL`, `WORDPRESS_USER`, `WORDPRESS_APP_PASSWORD`, then **Actions → Publish WordPress posts → Run workflow** (see [.github/workflows/publish-wordpress-posts.yml](../.github/workflows/publish-wordpress-posts.yml)).
